...
class CustomPrescriptiontest extends Fpdf
{
    ...
    { 
        $this->Image( public_path().'/img/logo2023.jpg',21,7,15,15,'JPG');
        $this->Ln(1);
        $this->SetFont('Arial', 'B', 10);
        //$this->Cell(130, -5, strtoupper('MA. CLARISSA UY-PATRIMONIO, MD, FPOGS'), 0, 0, 'C');
        $this->Cell(190, -9, strtoupper($this->data['profile']->name), 0, 0, 'C');
        ...
        $this->Cell(190, -11, strtoupper($this->data['profile']->specialization1), 0, 0, 'C');
        $this->Ln(5);
        $this->SetFont('Arial', '', 7);
        //$this->Cell(130, -13, strtoupper("Urogynecologyy & Pelvic Reconstructive Surgery"), 0, 0, 'C');
        $this->Cell(190, -15, strtoupper($this->data['profile']->specialization2), 0, 0, 'C');
        $this->Ln(5);
        $this->SetFont('Arial', 'B', 7);
        $this->Cell(83.5, -20, strtoupper("CLINIC ADDRESS"), 0, 0, 'C');
        $this->Ln(5);
        $this->SetFont('Arial', '', 7);
        $this->Cell(115, -25, strtoupper("Dr. Pablo O. Torre Memorial Hospital"), 0, 0, 'C');  
        $this->Ln(5);
        $this->Cell(107, -30, strtoupper("Room 777 Medical Arts Building "), 0, 0, 'C');  
        $this->Ln(5);
        $this->Cell(105.5, -35, strtoupper("B.S. Aquino Drive, Bacolod City"), 0, 0, 'C');  
        $this->SetFont('Arial', 'B', 7);
        $this->Cell(54, -50, strtoupper("CLINIC HOURS"), 0, 0, 'C');
        $this->Ln(5);
        $this->SetFont('Arial', '', 7);
        $this->Cell(245, -55, strtoupper("Mon - Fri - 10:00AM to 3:00 PM"), 0, 0, 'C');  
        $this->Ln(5);
        $this->Cell(238.5, -61, strtoupper("Sat /Holiday - 10:00AM to 12:00 PM"), 0, 0, 'C');  
        $this->Ln(5);
        $this->Cell(242, -66, strtoupper("Tel. No.: (034) 433-7331 Loc. 777"), 0, 0, 'C');  
        $this->Ln(5);
        $this->Cell(257.5, -71, strtoupper("Cell #: 0925-234-5678"), 0, 0, 'C');  
        ...
        $this->cell(2, 3, 'Name of Patient :', 0, 0,'R');
        $this->cell(100, 3, strtoupper($this->data['patient_detail']->patientname), 'B', 0,'L');
        //$this->MultiCell(30, 4, strtoupper('JUAN DELA CRUZ JUAN DELA CRUZ JUAN DELA CRUZ'), 'B', 'L');
        $this->SetFont('');
        $this->cell(-13, 3, '', 0, 0);
        $this->cell(25, 3, 'Sex :', 0, 0,'R');
        ...
        $this->cell(8, 3, '','' ,0, 0);
        $this->cell(180, 4, $this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address.$this->data['patient_detail']->address, 'B',0, 0);
        $this->Image(public_path() . '/img/rx.png', 20, 41, 4, 4, 'PNG');
    }
    ...
    public function mealHeader()
    {
        ...
        $this->Cell(50, 5, "Medicine Name", 'LTR', 0, 'C');
    ...
    public function Footer()
    {
        ...
        $this->SetFont('Arial', '', 7);
        $this->cell(175, 3, "License No:", '', 0, 'R');
        $this->cell(20, 3, $this->data['profile']->prc, 'B', 1, 'R');
        $this->cell(175, 3, "PTR. No.", '', 0, 'R');
