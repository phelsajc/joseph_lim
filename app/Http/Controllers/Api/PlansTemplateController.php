...
use App\Model\PlansTemplate;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Validator;

class PlansTemplateController extends Controller
{
    public function index()
    {
        try {
            $templates = PlansTemplate::active()
                ->orderBy('type', 'asc')
                ->orderBy('name', 'asc')
                ->get();
    ...
    public function store(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'name' => 'required|string|max:255|unique:plans_templates,name',
    ...
    {
        try {
            $template = PlansTemplate::find($id);
            ...
            $validator = Validator::make($request->all(), [
                'name' => 'required|string|max:255|unique:plans_templates,name,' . $id,
                'content' => 'required|string|max:5000',
            ]);
